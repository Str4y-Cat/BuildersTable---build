...
import type { Document, ItineraryItem, Traveler, Trip } from '@/types'
import { displayDateRange } from '@/lib/tripHelpers'
import { itemTypeLabel } from '@/lib/itemTypeStyles'

export type TripPdfOptions = {
  /** When set, title the PDF for this traveler and filter to their entries/docs */
  traveler?: Traveler
  items?: ItineraryItem[]
  documents?: Document[]
  filename?: string
...
/**
 * Build and download a client-side PDF for a trip (curator full trip or traveler slice).
 */
export function downloadTripPdf(trip: Trip, options: TripPdfOptions = {}): void {
  ...
  // Documents first (traveler preference / locked decision)
  y += 6
  ...
  if (!sortedItems.length) {
    ...
    for (const item of sortedItems) {
      if (item.date !== currentDate) {
        ...
        doc.setFont('helvetica', 'bold')
        doc.setFontSize(11)
        y = writeWrapped(doc, dayFmt.format(new Date(item.date)), margin, y, maxWidth, 6)
        doc.setFont('helvetica', 'normal')
        doc.setFontSize(10)
      }
      ...
      const typePart = itemTypeLabel(item.type)
      y = writeWrapped(
        doc,
        `• ${timePart}${item.title} (${typePart})`,
        margin,
        y,
        maxWidth
      )
  ...
  const filename =
    options.filename ?? `${slugify(trip.name)}-${who}.pdf`
