 ...
 * visuals. Deliberately NOT read from the trips store — the marketing page
 * must not depend on app state, and this content needs to stay stable while
 * the store is mutated by demo interactions.
 *
 * Names and details mirror `src/data/mockData.json` so a visitor who clicks
 * through to the demo sees the same trip they were just looking at.
 */
